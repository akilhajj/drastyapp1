...
import { GraduationCap, Globe, LogOut, ChevronRight } from 'lucide-react';
...
export default function Sidebar({ activeTab, onTabChange }: SidebarProps) {
  ...
  return (
    <aside
      className={`hidden lg:flex flex-col w-64 min-h-screen bg-navy-900/80 backdrop-blur-xl
        border-e border-white/10 fixed top-0 z-30 ${isRTL ? 'right-0' : 'left-0'}`}
    >
      {/* Logo */}
        ...
        {navItems.map(item => {
          ...
          return (
            ...
            >
              ...
              {active && (
                <ChevronRight className={`w-3 h-3 opacity-60 ${isRTL ? 'rotate-180' : ''}`} />
              )}
            </button>
          );
        })}
