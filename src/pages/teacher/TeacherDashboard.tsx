...
export default function TeacherDashboard({ activeTab }: TeacherDashboardProps) {
  return (
    <div className="animate-fade-in">
      {activeTab === 'teacher_home'  && <TeacherOverview />}
      {activeTab === 'tickets'       && <TeacherTickets />}
      {activeTab === 'consultations' && <TeacherConsultations />}
      {activeTab === 'studentLogs'   && <TeacherStudentLogs />}
...
function TeacherOverview() {
  ...
  useEffect(() => {
    const iv = setInterval(() => setShift(isShiftActive()), 60000);
        ...
        <div className="glass-card p-4 h-80 overflow-y-auto space-y-3">
          {messages.map(m => (
            <div key={m.id} className={`flex gap-2 ${m.sender_role === 'teacher' ? 'justify-end' : ''}`}>
              {m.sender_role !== 'teacher' && <div className="w-7 h-7 rounded-full bg-primary-600 flex items-center justify-center text-xs text-white shrink-0">S</div>}
              <div className={`max-w-xs rounded-2xl px-4 py-2 text-sm ${m.sender_role === 'teacher' ? 'bg-primary-600 text-white rounded-tr-sm' : 'bg-white/10 text-white/80 rounded-tl-sm'}`}>
                {m.message}
                <div className="text-xs opacity-50 mt-1">{new Date(m.created_at).toLocaleTimeString()}</div>
              </div>
              {m.sender_role === 'teacher' && <div className="w-7 h-7 rounded-full bg-emerald-600 flex items-center justify-center text-xs text-white shrink-0">T</div>}
            </div>
          ))}
          <div ref={msgEndRef} />
        </div>
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <button onClick={loadTickets} className="p-2 glass rounded-xl text-white/60 hover:text-white"><RefreshCw className="w-4 h-4" /></button>
      </div>
...
function TeacherConsultations() {
  ...
  async function load() {
    const { data } = await supabase.from('consultation_slots')
      ...
      .eq('teacher_id', user?.id).order('slot_date', { ascending: true });
    setSlots(data || []);
  }
  useEffect(() => { load(); }, [user]);
...
function TeacherMonitoring() {
  ...
  useEffect(() => {
    const iv = setInterval(() => setShift(isShiftActive()), 30000);
    return () => clearInterval(iv);
  }, []);
  ...
  useEffect(() => {
    ...
    supabase.from('student_quiz_attempts')
      ...
      .eq('passed', false).eq('manually_unlocked', false)
      .order('created_at', { ascending: false }).limit(30)
      .then(({ data }) => setAttempts(data || []));
  }, [shift]);

  async function unlockQuiz(id: string) {
    setUnlocking(id);
    await supabase.from('student_quiz_attempts').update({
      manually_unlocked: true, unlocked_by: user?.id, unlocked_at: new Date().toISOString(),
    }).eq('id', id);
        ...
        <div className="glass-card p-10 text-center max-w-md mx-auto">
          <div className="relative w-28 h-28 mx-auto mb-6">
            <svg viewBox="0 0 200 200" className="w-full h-full animate-float">
              <defs>
                <radialGradient id="lg1" cx="50%" cy="30%" r="60%">
                  <stop offset="0%" stopColor="#fbbf24" /><stop offset="100%" stopColor="#b45309" />
                </radialGradient>
                <radialGradient id="lg2" cx="50%" cy="20%" r="70%">
                  <stop offset="0%" stopColor="#60a5fa" /><stop offset="100%" stopColor="#1e40af" />
                </radialGradient>
              </defs>
              <path d="M70 90 Q70 40 100 40 Q130 40 130 90" stroke="url(#lg1)" strokeWidth="14" fill="none" strokeLinecap="round"/>
              <rect x="55" y="88" width="90" height="75" rx="12" fill="url(#lg2)"/>
          ...
          {attempts.length === 0
            ...
            : attempts.map(a => (
                <div key={a.id} className="flex items-center gap-3 p-3 rounded-xl bg-white/5">
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-medium text-white">{a.profiles?.full_name}</div>
                    <div className="text-xs text-white/40">{lang === 'ar' ? a.quizzes?.title_ar : a.quizzes?.title_en}</div>
